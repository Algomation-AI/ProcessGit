...
// DVSXMLContent returns typed DVS XML as plain text for the viewer shell.
func DVSXMLContent(ctx *context.Context) {
	...
	if fInfo.blobOrLfsSize >= setting.UI.MaxDisplayFileSize {
		ctx.JSON(http.StatusBadRequest, map[string]string{"error": "file is too large to render as XML"})
		return
	}
